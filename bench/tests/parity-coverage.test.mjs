...
/* Компоненты без своей темы: в библиотеке это обёртки над другими компонентами
   (ColorModeSelect — USelectMenu с тремя пунктами), поэтому ключа в themes.json у них нет.
   В ките они есть с раунда 126 (nuxt-ui-ds-qg2), и страница им нужна так же, как всем. */
const COMPOSED = ['color-mode-select']
...
test('у каждого компонента есть страница и оба артборда', async (t) => {
  ...
  const names = [...Object.keys(JSON.parse(await readFile('dist/docs/themes.json', 'utf8'))).filter((n) => !EXCLUDED(n)), ...COMPOSED]

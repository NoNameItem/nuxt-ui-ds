...
/* @nuxt/ui 4.11.1 — theme copied verbatim from themes/context-menu.ts (source/themes.json -> "context-menu").
...
/* DOM extracted from ContextMenu.vue — element nesting and data-slot names as shipped. */
const tree = [{"t":"div","s":"content","c":[{"t":"div","s":"viewport","if":1},{"t":"div","s":"group","if":1},{"t":"div","s":"label","if":1},{"t":"div","s":"separator","if":1},{"t":"div","s":"item","if":1},{"t":"div","s":"itemLeadingIcon","if":1},{"t":"div","s":"itemLeadingAvatar","if":1},{"t":"div","s":"itemLeadingAvatarSize","if":1},{"t":"div","s":"itemTrailing","if":1},{"t":"div","s":"itemTrailingIcon","if":1},{"t":"div","s":"itemTrailingKbds","if":1},{"t":"div","s":"itemTrailingKbdsSize","if":1},{"t":"div","s":"itemWrapper","if":1},{"t":"div","s":"itemLabel","if":1},{"t":"div","s":"itemDescription","if":1},{"t":"div","s":"itemLabelExternalIcon","if":1}]}];

function itemColorClass(color, active, slot) {
  return (contextMenuTheme.compoundVariants || [])
    .filter((cv) => cv.color === color && cv.active === active && cv.class && cv.class[slot])
    .map((cv) => cv.class[slot]).join(' ');
}

const render = createRenderer('context-menu', contextMenuTheme, tree, {
  portal: true,
  contentNeedsOpen: true,
  /* ContextMenu.vue:38-40 — the root tv() gets `size` only; the item's `active`
     is ULink's (ContextMenuContent.vue:167: item.active, else false without a
     route) and its colour is `item?.color` (applied in slotAttrs) */
